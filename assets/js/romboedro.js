...
document.getElementById('romboedro-form').addEventListener('submit', function (e) {
    e.preventDefault();
    const aresta = document.getElementById('aresta').value;

    if (aresta > 0) {
        const area = (6 * Math.pow(aresta, 2)).toFixed(2);
        document.getElementById('result').innerHTML = `<p class="text-success">A área do romboedro é: <strong>${area}</strong> m²</p>`;
    } else {
